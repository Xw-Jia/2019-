...
给定一个输入句`x =（x1; :::; xm）`，**如果一个单词`xi不属于 K`，我们就像以前一样嵌入它来获得wi**。
**但是，如果单词`xi属于K`，我们可以将此主题词嵌入为`ti∈Rd`，这是主题嵌入矩阵`Dtopic∈RK×d`中的一行**，其中K是主题词汇的大小。
嵌入矩阵Dtopic从相应的预训练主题分布矩阵归一化，其行与每个单词分配给每个主题的次数成比例。
在这种情况下，位置嵌入矢量也分别被添加到编码器和解码器元素，以获得最终的主题嵌入`r =（r1; :::; rm）`和`s =（s1; :::; sn）`。
