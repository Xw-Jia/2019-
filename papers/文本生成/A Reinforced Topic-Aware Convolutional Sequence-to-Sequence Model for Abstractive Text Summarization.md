...
更多细节将在第4节中给出。虽然文本的词汇表示为V，但我们假设K⊂V。
给定一个输入句x =（x1; :::; xm），如果一个单词xi 2 = K，我们就像以前一样嵌入它来获得wi。
但是，如果单词xi 2 K，我们可以将此主题词嵌入为ti 2 Rd，这是主题嵌入矩阵Dtopic 2 RK×d中的一行，其中K是主题词汇的大小。
嵌入矩阵Dtopic从相应的预训练主题分布矩阵归一化，其行与每个主题分配给每个主题的次数成比例。
在这种情况下，位置嵌入矢量也分别被添加到编码器和解码器元件，以获得最终的主题嵌入r =（r1; :::; rm）和s =（s1; :::; sn）。
