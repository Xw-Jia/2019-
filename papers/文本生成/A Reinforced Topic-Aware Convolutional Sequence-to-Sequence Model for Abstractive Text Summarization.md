...
在encoder端和decoder端都使用**6个卷积层**，所有的embeddings，包括初始化的embeddings以及decoder端结果的embeddings，它们维度有256维。我们将学习率设置为0.25，并且一旦验证ROUGE分数在每个时期之后停止增加直到学习率低于10-5，则将衰减率降低0.1。 我们首先用最大似然估计法来训练基本的可感知的主题卷积神经网络，然后训练最小混合目标函数，即强化学习对象$L_{rl}和最大似然函数L_{ml}。
